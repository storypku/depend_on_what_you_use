...
def get_includes_from_file(file: Path, defines: List[str]) -> List[Include]:
    ...
    translation_unit = TranslationUnit.from_source(file, defines, None, parse_options, None)
...
def get_relevant_includes_from_files(
    ...
    defines: Union[List[str], None],
) -> List[Include]:
    all_includes = []
    if files:
        for file in files:
            includes = get_includes_from_file(Path(file), defines)
